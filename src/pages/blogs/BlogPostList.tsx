    ...
    <div className='grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 my-8'>
      {posts.map((post) => (
        <Link to={`/blogs/${post.id}`} key={post.id} className='group'>
          ...
          <h2 className='text-xl font-semibold text-emeraldGreen mt-4 mb-2'>{post.title}</h2>
          <p>{post.content}</p>
          <span className='text-sm text-gray-500'>
            by <span className='font-bold'>{post.author}</span> on <span className='font-bold'>{moment(post.updatedAt).format('MMM DD, YYYY')}</span>
          </span>
        </Link>
      ))}
    </div>
